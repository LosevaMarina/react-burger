           ...
           export const Order = ({ order }) => {
             ...
             const ingredientsInfo = ingredientsOrder.map(item => ingredients.find(ing => item == ing._id));
             ...
             return (
               <Link to={`/profile/orders/${order.number}`} state={{ background: location }} className={style.order}>
                ...
                {!visible && (ingredientsInfo.map((item, index) => <OrderIngredient key={index} card={item} />))}
                    {visible && (visible.map((item, index) => <OrderIngredient key={index} card={item} />))}
